...
from app.api.kis_trading_routes import router as kis_trading_router  # ★ KIS 모의/실제투자
...
async def lifespan(app: FastAPI):
    ...
    try:
        ...
        pf_scheduler.start()
        print("[스케줄러] 가상포트 일괄 가격 갱신 등록 (매일 18:35 KST)")
    except Exception as e:
        print(f"[스케줄러] 가상포트 갱신 스케줄러 등록 실패 (무시): {e}")
...
app.include_router(kis_trading_router)  # ★ KIS 모의/실제투자
@app.get("/")
async def root():
